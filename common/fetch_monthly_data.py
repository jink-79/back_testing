"""
=============================================================
  SCRIPT 01B — FETCH MONTHLY DATA (ALL F&O STOCKS)

  Fetches MONTHLY OHLCV data directly from TradingView using tvDatafeed
  ...
  Also generates:
    data/monthly/failed_symbols.txt
...
def connect():
    if TV_USERNAME and TV_PASSWORD:
        ...
        print("WARNING: Guest mode gives limited bars.")
        print("TIP: Add TV_USERNAME + TV_PASSWORD for more history.\n")
        tv = TvDatafeed()
    return tv
...
def fetch_and_save(tv, original_symbol):
    ...
    print(f"Fetching {original_symbol} (TV: {tv_symbol}) — Monthly...")
    ...
    try:
        ...
        filename = f"{original_symbol.replace('&','_').replace('-','_')}_monthly.csv"
        filepath = os.path.join(OUTPUT_DIR, filename)

        df.to_csv(filepath)

        print(f"  OK    : {len(df)} monthly bars fetched")
...
def main():
    print("=" * 65)
    print("  SCRIPT 01B — FETCH MONTHLY DATA (ALL F&O STOCKS)")
    print("  Exchange : NSE")
    print("  TF       : Monthly (1M)")
    print(f"  Bars     : {BARS}")
    print("=" * 65 + "\n")
